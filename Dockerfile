...
# Optimized for Oracle ARM64 - Robust Build Version
...
ENV BUN_INSTALL="/data/.bun" VIRTUAL_ENV="/opt/venv"
ENV PATH="/opt/venv/bin:/usr/local/go/bin:/data/.bun/bin:/data/.bun/install/global/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:$PATH"
...
# 🦞 THE ULTIMATE FIX FOR EXIT CODE 127
# 1. Re-install essentials in the final stage
# 2. Force node-gyp into the global path
# 3. Explicitly tell Bun where the python binary is
...
# Use 'npm' to install OpenClaw - it handles native ARM64 better than global Bun
RUN npm install -g pnpm openclaw@${OPENCLAW_VERSION}
